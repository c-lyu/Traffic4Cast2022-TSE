...
CACHEDIR = Path("cache")
PROCESSED = "processed"
road_graph_folder = Path("data/road_graph")
#%%
...
def cal_dist(row, df_nodes):
    coor1 = df_nodes.loc[df_nodes['node_id']==int(row['identifier'].split(',')[0]), ['x','y']]
    lon1 = coor1['x'].values[0]
    lat1 = coor1['y'].values[0]
    
    coor2 = df_nodes.loc[df_nodes['node_id']==int(row['identifier'].split(',')[1]), ['x','y']]
...
def get_network_static(args):
    ...
    print('Loading data...')
    df_edges, df_nodes, rgss_df = load_road_graph(BASEDIR, args.city, skip_supersegments=False)
    df_static = pd.DataFrame()
    print('Summarizing supersegments...')
    edge_maxspeeds_kph = {}
    edge_lengths_m = {}
    for u, v, sl, lm in zip(df_edges["u"], df_edges["v"], df_edges[speed_limit_field], df_edges["length_meters"]):
    ...
    df_static['length'] = rgss_df.apply(lambda x: sum([e['length'] for e in x['edges']]), axis=1)
    
    df_edges['edge'] = df_edges[['u', 'v']].apply(tuple, axis=1)
    df_static['n_oneway'] = rgss_df.apply(lambda x: df_edges[(df_edges['edge'].isin([e['edge'] for e in x['edges']]))&(df_edges['oneway'])].shape[0], axis=1)
        
    cols = ['sl_%s'%x for x in ['mean','std','25','50','75','min','max']]
    df_static[cols] = rgss_df.apply(edge_stat, args=(df_edges, speed_limit_field), axis=1, result_type='expand')

    # calculate harversine
    df_static['haversine'] = rgss_df.apply(cal_dist, df_nodes=df_nodes, axis=1)
    # shortest travel time
    df_static['shortest_tt'] = rgss_df.apply(lambda x: sum([e['length']/e['max_speed'] for e in x['edges']]), axis=1)
