...
def create_features(sg_speed_support, nbrs, k, obj='test'):
    ...
    np.savez_compressed(PROCESSED+'/'+city+'/knn_eng_'+obj+'_p1'+str(k)+'_missing.npz', x_knn)
#%%
...
sg_speed = np.load(PROCESSED+'/'+city+'/x_sg.npz')['arr_0'] # need to run speed_features_fully.py first
#%%
