...
def read_knn_features(path, city, ks, obj, shape, p, zone="", missing=False):
    ...
    for k in ks:
        if missing:
            features = np.load(f"{path}/{city}/knn_eng_{obj}_{p}{k}_missing.npz")[
                "arr_0"
            ]
        else:
...
def read_speed_features(path, city, ks, obj, shape, p, missing=False):
    ...
    for k in ks:
        if missing:
            features = np.load(f"{path}/{city}/knn_eng_{obj}_speed_{p}{k}_missing.npz")[
                "arr_0"
            ]
        else:
            features = np.load(f"{path}/{city}/knn_eng_{obj}_speed_{p}{k}.npz")["arr_0"]
        knn_eng.append(features)
    knn_features = np.concatenate(knn_eng, axis=1)
    if obj == "train":
