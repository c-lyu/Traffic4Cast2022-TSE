...
def cal_speed_features(args, ks):
    ...
    for d in tqdm(range(num_days)):
        ...
        y_support = np.concatenate((y_eta[:d*num_t_per_day],y_eta[(d+1)*num_t_per_day:]), axis=0)
        ...
        sg_speed_support = np.concatenate((sg_speed[:d*num_t_per_day],sg_speed[(d+1)*num_t_per_day:]), axis=0)
    #%%
    ...
    for i, k in enumerate(ks):
        ...
        np.savez_compressed(PROCESSED+'/'+args.city+'/knn_eng_train_speed_p'+args.knn_p+str(k)+'.npz', x_knn)
        
        nbrs = knn.kneighbors(x_test, n_neighbors=k, return_distance=False)
        x_tmp = create_features(sg_speed, nbrs, k)
        np.savez_compressed(PROCESSED+'/'+args.city+'/knn_eng_test_speed_p'+args.knn_p+str(k)+'.npz', x_tmp)
#%%
