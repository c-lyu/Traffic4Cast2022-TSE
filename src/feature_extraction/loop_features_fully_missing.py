...
def loop_statistics(args):
    print('===Preparing %s... calculate_missing=%s, construct_test=%s' %(args.city, args.calculate_missing, args.construct_test))
    ...
    if args.calculate_missing:
        np.savez_compressed(PROCESSED + '/' + args.city + '/x_nodes_eta_missing.npz', x_nodes)
    else:
    ...
    if args.construct_test:
        ...
        x_nodes = np.concatenate((df_sum, df_mean, df_std, num_valid), axis=2)
        np.savez_compressed(PROCESSED + '/' + args.city + '/x_nodes_test_eta.npz', x_nodes)
#%%
