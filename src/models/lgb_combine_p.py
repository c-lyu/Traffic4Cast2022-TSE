...
from src.feature_extraction.read_knn_features import read_knn_features, read_speed_features
...
for city in cities:
    ...
    x_all = np.concatenate((x_knn1, x_knn2, x_static_train, x_nodes, x_sg_speed), axis=2)
    ...
    static_cols = ['n_nodes', 'length', 'n_oneway', 'sl_mean', 'sl_std', 'sl_25', 'sl_50',
       'sl_75', 'sl_min', 'sl_max', 'haversine', 'shortest_tt',\
        'mean_all','std_all','25_all','50_all','75_all','min_all','max_all','thre_tt_0','thre_tt_1','thre_tt_2','sg_id']
    ...
    speed_cols = []
    for k in [5, 10, 50]:
        for metric in ['median_speed','ffs','vol_class_1','vol_class_3','vol_class_5','vol_class_nan']:
            for f in ['mean','std']:
    ...
    ks = [5, 10, 30, 50, 100]
    x_knn_test1 = read_knn_features(PROCESSED, city, ks, 'test', y.shape, p='p1', zone='')
    x_knn_test2 = read_knn_features(PROCESSED, city, ks, 'test', y.shape, p='p2', zone='')
    
    # supersegment speed statistics
    ks = [5, 10, 50]
    x_sg_speed = read_speed_features(PROCESSED, city, ks, 'test', y.shape, p='p1')
    
    x_test = np.concatenate((x_knn_test1, x_knn_test2, x_static_test, x_nodes_test, x_sg_speed), axis=2)
