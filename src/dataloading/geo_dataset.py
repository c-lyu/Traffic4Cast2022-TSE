...
class T4c22GeomExtDataset(torch_geometric.data.Dataset):
    """Extended Dataset for t4c22 core competition (congestion classes) for one city.
    ...
    def __init__(
        ...
        self.nodes_valid = torch.tensor(
            df_nodes.counter_info.ne("").values, dtype=torch.bool
        )
    ...
    def get(self, idx: int) -> torch_geometric.data.Data:
        if self.split == "test":
            ...
            data = torch_geometric.data.Data(
                x=x, edge_index=self.edge_index, y=None, edge_attr=self.edge_attr_new
            )
        else:
            ...
            if self.x_imputed is not None:
                ...
                x[self.nodes_valid] = torch.tensor(
                    self.x_imputed[:, day_t_idx - 4 : day_t_idx]
                ).type_as(x)
            else:
                x = self.dataset[idx].x
            data = torch_geometric.data.Data(
                x=x, edge_index=self.edge_index, y=y, edge_attr=self.edge_attr_new
            )
        return data
