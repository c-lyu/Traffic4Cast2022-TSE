...
from t4c22.dataloading.t4c22_dataset import (
    T4c22Dataset as OfficalT4c22Dataset,
    T4c22Competitions,
)
...
class T4c22Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.torch_road_graph_mapping = TorchRoadGraphMapping(
            ...
            if self.day_t_filter is not None
            else None,
        ...
        if split == "test":
            ...
            self.day_t = [
                (day, t)
                for day in cc_dates(self.root, city=city, split=self.split)
                for t in range(4, 96)
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:  # noqa:C901
        ...
        if self.cachedir is not None:
            cache_file = (
                self.cachedir
                / f"inputs_{self.city}_{day}_{t}{self.temporal_features_suffix}.pt"
            )
            if cache_file.exists():
                x = torch.load(cache_file)
        if x is None:
            x = self.torch_road_graph_mapping.load_inputs_day_t(
                basedir=basedir, city=city, split=self.split, day=day, t=t, idx=idx
            )
        ...
        if self.cachedir is not None:
            cache_file = self.cachedir / (
                f"cc_labels_{self.city}_{day}_{t}.pt"
                if self.competition == T4c22Competitions.CORE
                else f"eta_labels_{self.city}_{day}_{t}.pt"
            )
            if cache_file.exists():
        ...
        if y is None:
            if self.competition == T4c22Competitions.CORE:
                y = self.torch_road_graph_mapping.load_cc_labels_day_t(
                    basedir=basedir, city=city, split=split, day=day, t=t, idx=idx
                )
            else:
                y = self.torch_road_graph_mapping.load_eta_labels_day_t(
                    basedir=basedir, city=city, split=split, day=day, t=t, idx=idx
                )
