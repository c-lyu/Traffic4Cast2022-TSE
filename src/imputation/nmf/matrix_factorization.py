...
class DataImputation(object):
    ...
    def update_parameters(self):
        ...
        for i in range(self.n_venues):
            V_j = V[:, self.R[i, :] > 0]
            U[:, i] = np.dot(
                np.linalg.inv(np.dot(V_j, V_j.T) + lambda_U * np.identity(self.n_dims)),
                np.dot(self.R[i, self.R[i, :] > 0], V_j.T),
            )

        for j in range(self.n_slices):
            U_i = U[:, self.R[:, j] > 0]
            V[:, j] = np.dot(
                np.linalg.inv(np.dot(U_i, U_i.T) + lambda_V * np.identity(self.n_dims)),
                np.dot(self.R[self.R[:, j] > 0, j], U_i.T),
            )
    ...
    def log_a_posteriori(self):
        ...
        return -0.5 * (
            np.sum(np.dot(R_UV, R_UV.T))
            + lambda_U * np.sum(np.dot(U, U.T))
            + lambda_V * np.sum(np.dot(V, V.T))
        )
    ...
    def evaluate(self, data_true):
        ...
        rmse = np.sqrt(
            sum((data_true[value_index] - data_estimated[value_index]) ** 2) / num
        )
        mape = (
            sum(
                np.absolute(
                    (data_true[value_index] - data_estimated[value_index])
                    / data_true[value_index]
                )
            )
            / num
        )
