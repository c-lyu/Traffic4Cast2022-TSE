...
def train_imputation(x, batch_size=3000):
    ...
    for bat in range(n_batch):
        ...
        (
            ...
            mape_train,
            parameters,
            data_estimated_impute,
        ) = impute.train(x_batch)
...
if __name__ == "__main__":
    ...
    X_support_sample, y_support_sample = pd.read_pickle(
        "../../../traffic4cast/T4C_INPUTS_2022/data_sample.pckl"
    )
    del y_support_sample
    n_dayts = X_support_sample.shape[0]
    x_raw, x_imputed, rmse_train, mape_train = train_imputation(
        X_support_sample, n_dayts, batch_size=1000
    )
